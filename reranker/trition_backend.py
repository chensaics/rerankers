...
"""
@Author:     sai.chen
@FileName:   rerank_backend.py
@Date:       2024/08/31
@Description:
===============================================
rerank model backend
    Trition with URL
    Haggingface 加载模型方式
===============================================
"""
...
# G:\models\bge-reranker-v2-m3
class RerankerTritonBackend:
    def __init__(self):
        tokenizer_path = "/mnt/g/models/bge-reranker-v2-m3"
    ...
    def inference(self, serialized_inputs):
        ...
        for input_name, data in serialized_inputs.items():
            infer_input = grpcclient.InferInput(
                input_name, data.shape, grpcclient.np_to_triton_dtype(data.dtype)
            )
        ...
        # logger.info("local rerank infer time: {} s".format(time.time() - start_time))

        # 获取响应数据
        result_data = response.as_numpy(output_name)
        # logger.info(f"rerank res: {result_data}")
    ...
    def tokenize_preproc(
        ...
        for pid, passage in enumerate(passages):
            passage_inputs = self.tokenizer.encode_plus(
                passage, truncation=False, padding=False, add_special_tokens=False
            )
    ...
    def predict(
        ...
        # logger.info(f"merge_tot_scores: {merge_tot_scores}")
